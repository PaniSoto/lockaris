...
/**
 * POST: Recuperación "Just-in-Time" de datos sensibles.
 * No enviamos contraseñas en el GET general para minimizar exposición.
 * Se usa POST para evitar que datos sensibles queden en logs de URLs.
 */
export async function POST(request, { params }) {
  try {
    ...
    if (credential.type === "NOTE") {
      return NextResponse.json({
        notes: decrypt(credential.notes, credential.iv),
      });
    }

    return NextResponse.json({
      password: decrypt(credential.encryptedPassword, credential.iv),
    });
  } catch (error) {
 ...
 * Usa deleteMany con userId para asegurar que un usuario no borre claves ajenas.
 */
...
/**
 * PUT: Actualización con re-encriptación.
 * Si el dato sensible cambia, generamos un nuevo IV (Initialization Vector) para máxima seguridad.
 */
