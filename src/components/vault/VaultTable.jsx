        ...
        {items.map((item) => (
          ...
          >
            ...
            <div className="flex items-center gap-4">
              <div
                className={`h-11 w-11 rounded-xl flex items-center justify-center shadow-sm border ${
                  item.type === "CARD"
                ...
                <span className="text-xs font-medium uppercase tracking-wider text-gray-400">
                  {item.type === "CARD"
                    ...
                    : item.type === "NOTE"
                      ...
                      : "Inicio de sesión"}
                  {item.username && (
                    <span className="lowercase text-gray-300">
                      {" "}
                      • {item.username}
                    </span>
                  )}
            ...
            {/* Iconos de estado y botones de 3 puntos */}
                ...
                {activeMenu === item.id && (
                  <div className="absolute right-0 top-10 w-56 bg-white border border-gray-200 rounded-xl shadow-2xl z-50 py-1 overflow-hidden animate-in fade-in zoom-in duration-100">
                    ...
                    {item.type === "CARD" ? (
                      <>
                        ...
                        >
                          ...
                          {copiedId === `cardNumber-${item.id}` && (
                            <Check size={14} className="text-green-500" />
                          )}
                        </button>
                        <button
                          onClick={(e) =>
                            handleCopySensitive(e, item.id, "CARD", "cvv")
                          }
                          className="w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center justify-between border-b border-gray-100"
                        >
                          <span className="flex items-center gap-2">
                            {isFetchingSensitive === item.id ? (
                      ...
                      <button
                        onClick={(e) =>
                          handleCopySensitive(e, item.id, "NOTE", "notes")
                        }
                        className="w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center justify-between border-b border-gray-100"
                      >
                        <span className="flex items-center gap-2">
                      ...
                      <>
                        <button
                          onClick={(e) =>
                            handleCopySimple(e, item.username, item.id, "user")
                          }
                          className="w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center justify-between"
                        >
                          <span className="flex items-center gap-2">
                            <Copy size={14} /> Copiar usuario
                          </span>
                        ...
                        <button
                          onClick={(e) =>
                            handleCopySensitive(e, item.id, "LOGIN", "password")
                          }
                          className="w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center justify-between border-b border-gray-100"
                        >
                          <span className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2 mt-1"
                    >
